...
namespace SoundRecognition
{
    ...
    public class Figure
    {
        ...
        private Graphics mGfxFrame;

        private Bitmap mBmpGraph { get; set; }
        ...
        public Figure(int width, int height)
        {
            ...
            mGfxFrame.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.AntiAlias;
            mGfxFrame.TextRenderingHint = System.Drawing.Text.TextRenderingHint.AntiAlias;
        ...
        public void Resize(int width, int height)
        {
            ...
            mBmpFrame = new Bitmap(width, height);
            mGfxFrame = Graphics.FromImage(mBmpFrame);

            // now re-calculate the graph size based on the padding
            FramePad(null, null, null, null);

            // now resize the graph bitmap
            mBmpGraph = new Bitmap(mBmpFrame.Width - padL - padR, mBmpFrame.Height - padT - padB);
            gfxGraph = Graphics.FromImage(mBmpGraph);

            // now resize axis to the new pad dimensions
            xAxis.Resize(mBmpGraph.Width);
            yAxis.Resize(mBmpGraph.Height);
        }
        ...
        public void FrameRedraw()
        {
            try
            {
                ...
                int posB = mBmpGraph.Height + padT;
                int posCx = mBmpGraph.Width / 2 + padL;
                int posCy = mBmpGraph.Height / 2 + padT;

                int tick_size_minor = 2;
                int tick_size_major = 5;

                // draw the data rectangle and ticks
                mGfxFrame.DrawRectangle(penAxis, graphPos.X - 1, graphPos.Y - 1, mBmpGraph.Width + 1, mBmpGraph.Height + 1);
                mGfxFrame.FillRectangle(new SolidBrush(colorGraphBg), graphPos.X, graphPos.Y, mBmpGraph.Width, mBmpGraph.Height);
                foreach (Axis.Tick tick in xAxis.ticksMajor)
                    mGfxFrame.DrawLine(penAxis, new Point(padL + tick.posPixel, posB + 1), new Point(padL + tick.posPixel, posB + 1 + tick_size_minor));
                foreach (Axis.Tick tick in yAxis.ticksMajor)
                    mGfxFrame.DrawLine(penAxis, new Point(padL - 1, padT + tick.posPixel), new Point(padL - 1 - tick_size_minor, padT + tick.posPixel));
                foreach (Axis.Tick tick in xAxis.ticksMinor)
                {
                    mGfxFrame.DrawLine(penGrid, new Point(padL + tick.posPixel, padT), new Point(padL + tick.posPixel, padT + mBmpGraph.Height - 1));
                    mGfxFrame.DrawLine(penAxis, new Point(padL + tick.posPixel, posB + 1), new Point(padL + tick.posPixel, posB + 1 + tick_size_major));
                    mGfxFrame.DrawString(tick.label, fontTicks, brush, new Point(tick.posPixel + padL, posB + 7), sfCenter);
                }
                foreach (Axis.Tick tick in yAxis.ticksMinor)
                {
                    mGfxFrame.DrawLine(penGrid, new Point(padL, padT + tick.posPixel), new Point(padL + mBmpGraph.Width, padT + tick.posPixel));
                    mGfxFrame.DrawLine(penAxis, new Point(padL - 1, padT + tick.posPixel), new Point(padL - 1 - tick_size_major, padT + tick.posPixel));
                    mGfxFrame.DrawString(tick.label, fontTicks, brush, new Point(padL - 6, tick.posPixel + padT - 7), sfRight);
                }

                // draw labels
                mGfxFrame.DrawString(labelX, fontAxis, brush, new Point(posCx, posB + 24), sfCenter);
                mGfxFrame.DrawString(labelTitle, fontTitle, brush, new Point(mBmpFrame.Width / 2, 8), sfCenter);
                mGfxFrame.TranslateTransform(mGfxFrame.VisibleClipBounds.Size.Width, 0);
                mGfxFrame.RotateTransform(-90);
                mGfxFrame.DrawString(labelY, fontAxis, brush, new Point(-posCy, -mBmpFrame.Width + 2), sfCenter);
                mGfxFrame.ResetTransform();
        ...
        public void GraphClear()
        {
            try
            {
                gfxGraph.DrawImage(mBmpFrame, new Point(-padL, -padT));
                pointCount = 0;
            }
            catch
            {
        ...
        private string benchmarkMessage
        {
            get
            {
                ...
                string msg = "";
                double imageSizeMB = mBmpFrame.Width * mBmpFrame.Height * 4.0 / 1024 / 1024;
                msg += string.Format("{0} x {1} ({2:0.00} MB) ", mBmpFrame.Width, mBmpFrame.Height, imageSizeMB);
        ...
        {
            Bitmap bmpMerged = null;
            try
            {
                bmpMerged = new Bitmap(mBmpFrame);
                Graphics gfx = Graphics.FromImage(bmpMerged);
                gfx.DrawImage(mBmpGraph, graphPos);
                ...
                if (this.stopwatch.ElapsedTicks > 0)
                {
                    ...
                    Point pointStamp = new Point(mBmpFrame.Width - padR - 2, mBmpFrame.Height - padB - 14);
                    StringFormat sfRight = new StringFormat();
                    sfRight.Alignment = StringAlignment.Far;
        ...
        public void PlotSignal(double[] values, double pointSpacing = 1, double offsetX = 0, double offsetY = 0, float lineWidth = 1, Color? lineColor = null)
        {
            try
            {
                ...
                if (dataPointsPerPixel < 1)
                {
                    ...
                    int iRight = iLeft + (int)(dataPointsPerPixel * mBmpGraph.Width);
                ...
                else
                {
                    ...
                    for (int xPixel = Math.Max(0, dataMinPx); xPixel < Math.Min(mBmpGraph.Width, dataMaxPx); xPixel++)
                    {
                ...
                {
                    Console.WriteLine("Exception plotting:" + e.Message);
